function [D,G,C,I,M]=build_ops2(Grid)
% author: someone
% date: someday
% description:
...
if (Nx>1) && (Ny==1) % 1D x-direction
    M = 0.5 .* spdiags([ones(Nx, 1) ones(Nfx, 1)],[-1,0],Nx+1,Nx);  % 1D mean-matrix in x-dir
    M(1,1) = 1; 
    M(Nx+1,Nx) = 1;
elseif (Nx==1) && (Ny>1) % 1D y-direction
    M = 0.5 .* spdiags([ones(Ny,1) ones(Nfy,1)],[-1 0],Ny+1,Ny);   % 1D mean-matrix in y-dir
    M(1,1) = 1; 
    M(Ny+1,Ny) = 1;
elseif (Nx>1) && (Ny>1)  % 2D case
    % 1D operators
    Mx = 0.5 .* spdiags([ones(Nx, 1) ones(Nx, 1)],[-1 0],Nx+1,Nx); % 1D mean-matrix in x-dir
    Mx(1,1) = 1; 
    Mx(Nx+1,Nx) = 1;
    My = 0.5 .* spdiags([ones(Ny,1) ones(Ny,1)],[-1 0],Ny+1,Ny); % 1D mean-matrix in y-dir
    My(1,1) = 1; 
    My(Ny+1,Ny) = 1;
    % 2D operators
